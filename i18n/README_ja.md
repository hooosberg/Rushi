...
- **真にローカル** —— アカウント不要、クラウド送信なし、解析 SDK なし。現バージョンはすべてのデータをアプリサンドボックス内にのみ保存します。アンインストールでデータは完全に削除されます。
...
| データ保存 | アプリサンドボックスのみ（アンインストールで完全に削除） |
...
iOS アプリは Swift 5 / SwiftUI、iOS 17 で構築。ローカル保存に SwiftData、経文組版に CoreText（iOS 17 の `UIFont(name:)` バグを回避する独自の中華セリフフォールバック、`CTFontCreateWithName` 経由）、挂飾の揺れ感に CoreMotion を使用。Swift ソースコードはまだオープンソース化されていません。本リポジトリは公開ウェブサイトと経典コーパスのみを含みます。
